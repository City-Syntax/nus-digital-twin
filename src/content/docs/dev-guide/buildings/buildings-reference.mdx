...
A `string` value can also be used. String values will be displayed as it is.
...
Type: `number` or `Array<{ label: 'Heating' | 'Cooling', value: number }>`
...
Different values can also be defined for heating and cooling as shown in the example below. Each label will be displayed with its corresponding values on a single row.

```json title="Multiple Values Example" frame=terminal
{
  "thermostatSetPoint": [
    { "label": "Heating", "value": 25 },
...
A number can be specified. This number represents the number of hours per week. The example below defines a core outside air schedule of 50 hours per week.

```json title=Example frame=terminal
{
  "coreOutsideAirSchedule": 50
}
```
...
A number can be specified. This number represents the number of hours per week. The example below defines a perimeter outside air schedule of 50 hours per week.

```json title=Example frame=terminal
{
  "perimeterOutsideAirSchedule": 50
}
```
